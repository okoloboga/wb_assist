...
### 🚨 **ПРОБЛЕМЫ:**
1. **НЕТ полей:** `price`, `discount_price`, `rating`, `reviews_count` - эти данные НЕ приходят из API товаров
2. **НЕ используем:** `imtID`, `nmUUID`, `subjectID`, `description`, `dimensions`, `characteristics`
3. **НЕПРАВИЛЬНО:** Пытаемся получить `price`, `rating` из API товаров, но их там нет
...
### 🚨 **ПРОБЛЕМЫ:**
1. **НЕ используем:** `quantityFull`, `category`, `subject`, `Price`, `Discount`, `isSupply`, `isRealization`, `SCCode`
2. **НЕПРАВИЛЬНО:** Пытаемся получить `name` из API остатков, но его там нет
3. **НЕТ поля:** `warehouse_id` - нет в API, но есть в модели
...
### 🚨 **ПРОБЛЕМЫ:**
1. **НЕ используем:** `pros`, `cons`, `state`, `productDetails.productName`, `productDetails.supplierArticle`, `productDetails.supplierName`, `productDetails.brandName`, `productDetails.size`, `video`, `wasViewed`, `photoLinks`, `userName`, `matchingSize`, `bables`, `lastOrderShkId`, `lastOrderCreatedAt`, `color`, `subjectId`, `subjectName`
2. **НЕПРАВИЛЬНО:** Пытаемся получить `updated_date` из API отзывов, но его там нет
...
#### **1. ЗАКАЗЫ - ✅ ИСПРАВЛЕНО И ПРОТЕСТИРОВАНО:**
...
#### **2. ТОВАРЫ - ✅ ИСПРАВЛЕНО И ПРОТЕСТИРОВАНО:**
...
#### **3. ОСТАТКИ - ✅ ИСПРАВЛЕНО И ПРОТЕСТИРОВАНО:**
...
#### **4. ОТЗЫВЫ - ✅ ИСПРАВЛЕНО И ПРОТЕСТИРОВАНО:**
...
#### **2. ИСПРАВИТЬ МАППИНГ:**
- Использовать все доступные поля из WB API
- Убрать попытки получить несуществующие поля
- Правильно сопоставлять данные

#### **3. ДОБАВИТЬ РАСЧЕТНЫЕ ПОЛЯ:**
