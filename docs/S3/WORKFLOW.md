...
```sql
SELECT * FROM wb_orders 
WHERE cabinet_id = ? AND updated_at > ? AND status = 'buyout'
```
...
```sql
SELECT * FROM wb_orders 
WHERE cabinet_id = ? AND updated_at > ? AND status = 'return'
```

**5. Негативные отзывы:**
```sql
SELECT * FROM wb_reviews 
WHERE cabinet_id = ? AND created_at > ? AND rating <= 3
```
...
**Версия:** 2.2 (Финальные исправления)  
**Статус:** РАБОТАЕТ
