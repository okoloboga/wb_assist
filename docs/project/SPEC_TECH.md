...
- **Основной сервис:** Монолит (WB API + уведомления + Bot API + Google Sheets интеграция)
- **Дополнительный сервис:** AI-сервис (отдельный микросервис для работы с OpenAI, в будущем — RAG)
...
│                 │    │                           │
│ • WB API        │    │ • OpenAI API (Text Gen)   │
│ • Notifications │    │ • Nano Banana (Image Gen) │
│ • Bot API       │    │ • AI Analysis / RAG       │
...
- **Архитектура:** Интегрирован в основной сервис (монолит).

### **OpenAI API (микросервис)**
- **Модель:** GPT-4 (актуальная версия)
- **Функции:** 
  - Генерация текстов карточек
  - AI-аналитика и рекомендации
  - Чат-ассистент с лимитами
- **Планы:** Расширение до RAG-архитектуры для более глубокого анализа.
- **Архитектура:** Отдельный микросервис.

### **Nano Banana API (планируется)**
- **Функции:**
...
- **Архитектура:** Часть единого AI-микросервиса.
...
### ✅ **Этап 3: Система уведомлений** - **ГОТОВ**
...
### **Этап 4: Google Sheets интеграция** - **ГОТОВ**
...
### ✅ **Этап 3: Система уведомлений (4 дня) - ГОТОВ**
- Единая синхронизация с WB API
...
### ✅ **Этап 4: Google Sheets интеграция (5 недель) - ГОТОВ**
- Интеграция с Google Sheets API
- OAuth 2.0 аутентификация
- Модели данных и шаблоны
- API для выгрузки данных из основного сервиса
- **Статус:** 100% готов, протестирован

### 🔄 **Этап 5: AI-сервис: Контент, Аналитика, Изображения (6 недель) - В РАЗРАБОТКЕ**
- **Генерация текста:**
  - **Новый флоу:** Сбор семантического ядра из описаний товаров конкурентов, на основе которого генерируется текст для карточки.
  - Текущая реализация написания текста по фото и описанию требует доработки.
- **AI-аналитика:**
  - Требуется обновление промптов и формата предоставляемых данных.
- **Генерация изображений (планируется):**
  - Интеграция с `Nano Banana` для генерации фото и инфографики.
- **Статус:** 45% готов (основа для AI-сервиса создана, ключевые фичи требуют реализации/доработки).

### ⏳ **Этап 6: Расширение AI-сервиса (RAG) - ПЛАНИРУЕТСЯ**
- Внедрение Retrieval-Augmented Generation для более точных и контекстуальных ответов AI-ассистента.
- Развитие архитектуры AI-микросервиса.

### ⏳ **Этап 7: Управление отзывами (4 дня) - ПЛАНИРУЕТСЯ**
- Уведомления о новых отзывах
- Автоответы (4–5★)
- Шаблоны стилей
...
### **Приоритет 1: Завершение Этапа 5 (AI-сервис)**
- Реализовать новый флоу генерации текста (сбор семантического ядра).
- Обновить промпты для AI-аналитики.
- **Срок:** 2-3 недели

### **Приоритет 2: Интеграция генерации изображений**
- Подключить API `Nano Banana` в рамках Этапа 5.
- Реализовать сохранение и доступ к сгенерированным изображениям.
- **Срок:** 2 недели

### **Приоритет 3: Начало работы над Этапом 6 (RAG)**
- Исследовать и спроектировать архитектуру для Retrieval-Augmented Generation.
- **Срок:** 3-4 недели

### **Приоритет 4: Планирование следующих этапов**
- Актуализировать требования и спланировать реализацию отложенных задач, начиная с "Управления отзывами".
- **Срок:** После завершения ключевых AI-функций
...
*Готовность: ~60% (Этапы 1-4 готовы, Этап 5 в разработке)*  
*Актуализация: Дополнено на основе сессии 12.11.2025*
