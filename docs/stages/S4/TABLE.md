...
**Особенности:**
- Одна строка = один товар с одним размером
- Колонки F-AB (или больше) содержат даты за последние 28 дней (4 недели назад до сегодня)
- В каждой колонке дня указывается количество заказов за этот день для данного товара/размера
- Колонка "Запас на Дней" рассчитывается как: `Остаток товара / (Заказы за месяц / 30)`
- Последняя колонка "СУММА" содержит сумму всех заказов за 28 дней

**Формат дат в заголовках:**
- Заголовки колонок содержат даты в формате `DD.MM.YYYY` (например: `10.10.2025`, `11.10.2025`, ...)
- Даты идут от 28 дней назад до сегодня (всего 28 колонок)
...
- В разрезе дня: 5 базовых полей + 28 колонок дней + 1 колонка СУММА = 34 поля (включая фото)
- **Итого: 76 полей данных**
...
- **Всего: 76 полей из PostgreSQL БД с JOIN на wb_products**
