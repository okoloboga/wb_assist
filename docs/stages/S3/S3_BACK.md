...
Полнофункциональная система уведомлений для отслеживания событий Wildberries с поддержкой общих кабинетов и webhook архитектуры. Система работает в реальном времени с настраиваемыми пользователем параметрами.
...
├── webhook_sender.py           # Webhook отправка уведомлений (новый)
├── models.py                   # Модели БД
...
5. **WebhookSender** отправляет уведомления боту через HTTP POST webhook
6. **Bot** получает webhook и отправляет уведомления пользователю в Telegram
...
### **Webhook API**
```bash
# Webhook endpoint для получения уведомлений от сервера
...
Content-Type: application/json
{
  ...
  "telegram_text": "Форматированный текст"
}
```
...
### **1. Тест webhook системы:**

#### **1.1 Отправка тестового webhook уведомления:**
```bash
...
curl -X POST -H "Content-Type: application/json" \
     ...
     "http://localhost:8000/webhook/notifications"
```

#### **1.2 Тест различных типов уведомлений:**
```bash
# Уведомление о выкупе заказа
curl -X POST -H "Content-Type: application/json" \
     -d '{
       "type": "order_buyout",
       "data": {...},
...
2. Проверить webhook endpoint: `POST /webhook/notifications`
...
2. Проверить записи в `NotificationHistory`
...
### **Новые возможности:**
- **Webhook архитектура** - переход с polling на webhook систему
...
- **Поддержка изображений** - отображение `image_url` в уведомлениях и детальном просмотре

### **Исправления:**
- ✅ Переход с polling на webhook архитектуру
...
- ✅ Добавлена поддержка `image_url` в уведомлениях и детальном просмотре
...
- `server/app/features/notifications/webhook_sender.py` - webhook отправка
- `bot/handlers/webhook.py` - webhook обработчики
...
- **Webhook architecture** - реальное время уведомлений
...
- **Image support** - поддержка изображений товаров
- **Code cleanup** - удалены неиспользуемые функции и дублирование

---

*Версия: S3.06*  
*Статус: ✅ Полностью реализовано + Webhook архитектура*  
*Архитектура: Webhook + TimezoneUtils + Rate Limiting + Image Support*  
*Готовность: 100%*  
*Новые возможности: Webhook архитектура, TimezoneUtils, улучшенный rate limiting, защита от дублирования, умная детекция остатков, поддержка изображений*
