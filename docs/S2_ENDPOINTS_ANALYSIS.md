...
## 🚨 Проблемы с текущими эндпоинтами

### **1. Заказы - неполное использование данных:**
- ✅ Получаем: `warehouseName`, `spp`, `finishedPrice`, `discountPercent`
- ❌ НЕ сохраняем: Отсутствуют поля в модели БД
- ❌ Результат: Показываем заглушки "Неизвестно" и нули

### **2. Комиссии - проблемы с сопоставлением:**
- ✅ Получаем: Полный список комиссий по категориям
- ❌ Проблема: Неправильное сопоставление с заказами
- ❌ Результат: `commission_percent=0.0` для многих заказов

### **3. Отзывы - неполное использование:**
- ✅ Получаем: Отзывы с оценками
- ❌ НЕ используем: Для расчета рейтингов товаров
- ❌ Результат: `rating=0.0` для всех товаров

## 🎯 Рекомендации

### **1. Исправить модель БД:**
- Добавить поля для складов, СПП, цен в `wb_orders`
- Добавить поля для рейтингов в `wb_products`

### **2. Улучшить маппинг данных:**
- Использовать все поля из WB API
- Правильно сопоставлять комиссии с заказами
...
**Проблемы:** Неполное использование данных WB API
