...
1. **Ключевые метрики** (`key_metrics`):
   ...
   - **ПОКАЗАТЕЛИ ЗА N ДНЕЙ (из `daily_trends.aggregates`):**
     - Доли: buyout_rate (% выкупа от заказов), return_rate (% возвратов от выкупов)
...
3. **Инсайты** (`insights`):
   ...
   - Эффективность выкупа (buyout_rate за период)
   - Влияние возвратов на бизнес (return_rate за период)
